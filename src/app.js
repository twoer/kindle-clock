...
;(function () {
  ...
  function setRemUnit () {
    // 计算方式参照 https://iclock.web.cloudendpoint.cn/
    var rem = Math.ceil(Math.min(screen.width, screen.height) / 600 * 20)
...
;(function () {
  ...
  // 时间
  const $elTime = document.getElementsByClassName('time')[0]
  const $elDate = document.getElementsByClassName('date')[0]
  const $elLunar = document.getElementsByClassName('lunar')[0]
  const loop = () => {
    const now = new Date()
    const timezoneOffset = now.getTimezoneOffset() + 480
    ...
    $elLunar.innerHTML = `${calendarJson.gzYear}年 ${calendarJson.Animal} ${calendarJson.IMonthCn}${calendarJson.IDayCn}`
  }
  loop()
  setInterval(() => {
    loop()
